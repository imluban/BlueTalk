...
class WifiDirectService(private val context: Context, private val listener: Listener) {
    ...
    fun register() { context.registerReceiver(receiver, filter) }
    ...
    fun discoverPeers() {
        ...
        if (Build.VERSION.SDK_INT >= 33) {
            // Android 13+ may require NEARBY_WIFI_DEVICES depending on use
        }
        manager.discoverPeers(channel) { status ->
            val msg = if (status == WifiP2pManager.SUCCESS) "Wi‑Fi Direct: discovering peers…" else "Wi‑Fi Direct: discover failed"
    ...
    fun connect(device: WifiP2pDevice) {
        ...
        manager.connect(channel, config) { status ->
            val msg = if (status == WifiP2pManager.SUCCESS) "Wi‑Fi Direct: connecting to ${device.deviceName}" else "Wi‑Fi Direct: connect failed"
            listener.onStatus(msg)
        }
    }
}
